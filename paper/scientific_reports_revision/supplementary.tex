...
\end{tabular}
\\[2pt]
\footnotesize The control condition is defined by perturbation label, so the cohort spans every cell line in the resource. The cohort reported here was reproduced from the extraction procedure under its recorded seed and validated against the per-cell gene counts recorded when the activations were written.
\end{table}
...
\begin{table}[H]
\centering
\caption{\textbf{Sensitivity of the SAE-vs-SVD comparison to the cosine threshold (Geneformer, all 18 layers).} For each layer and each cosine threshold $\tau \in \{0.3, 0.5, 0.7, 0.9\}$: percentage of alive SAE features whose decoder column has maximum absolute cosine above $\tau$ with any of the top-50 SVD axes (columns ``aligned\%''), and percentage of ontology enrichments found exclusively in the non-aligned (``novel'') set (columns ``\%enrich novel''). The value $\tau{=}0.5$ reproduces the per-layer SVD-aligned counts in Table~1 of the main text (the pipeline actually used $\tau{=}0.5$; the pipeline threshold is $\tau{=}0.5$). Across the full range, $>98\%$ of ontology enrichments live exclusively in the non-aligned set, at every threshold tested.}
\label{tab:svd_sweep}
\smallskip
\small
\begin{tabular}{r|rrrr|rrrr}
\toprule
& \multicolumn{4}{c|}{Aligned \%} & \multicolumn{4}{c}{\% enrich.\ from non-aligned} \\
...
\IfFileExists{figures/gb_figS3_scgpt_highways.pdf}{\includegraphics[width=\textwidth]{figures/gb_figS3_scgpt_highways.pdf}}
...
\begin{figure}[htbp]\centering
\IfFileExists{figures/gb_figS4_umap_overview.pdf}{\includegraphics[width=\textwidth]{figures/gb_figS4_umap_overview.pdf}}
...
\begin{figure}[htbp]\centering
\IfFileExists{figures/gb_figS5_l11_detail.pdf}{\includegraphics[width=\textwidth]{figures/gb_figS5_l11_detail.pdf}}
...
\begin{figure}[htbp]\centering
\IfFileExists{figures/gb_figS6_annotation_projections.pdf}{\includegraphics[width=\textwidth]{figures/gb_figS6_annotation_projections.pdf}}
